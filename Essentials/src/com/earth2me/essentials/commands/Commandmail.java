...
public class Commandmail extends EssentialsCommand {
    ...
    public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception {
        ...
        if (args.length >= 3 && "send".equalsIgnoreCase(args[0])) {
            ...
            if (user.isMuted()) {
                throw new Exception(user.tl("voiceSilenced"));
            }
    ...
    protected void run(Server server, CommandSource sender, String commandLabel, String[] args) throws Exception {
        if (args.length >= 1 && "read".equalsIgnoreCase(args[0])) {
            ...
            u.addMail(u.tl("mailFormat", "Server", getFinalArg(args, 2)));
            sender.sendTl("mailSent");
            return;
        } else if (args.length >= 2 && "sendall".equalsIgnoreCase(args[0])) {
            ess.runTaskAsynchronously(new SendAll("mailFormat", "Server", getFinalArg(args, 1)));
            sender.sendTl("mailSent");
            return;
            ...
            u.addMail(u.tl("mailFormat", "Server", getFinalArg(args, 1)));
            sender.sendTl("mailSent");
            return;
        }
        throw new NotEnoughArgumentsException();
